...
void Image::NonParallelSmoothing(size_t core_radius) {
	...
	for (int64_t index_y = core_radius; index_y + core_radius < size_y_; ++index_y) {
		for (int64_t index_x = core_radius; index_x + core_radius < size_x_; ++index_x) {
			median_red = 0, median_green = 0, median_blue = 0;
			auto start = omp_get_wtime();
			for (int64_t core_index_y = index_y - core_radius; core_index_y <= index_y + core_radius; ++core_index_y) {
				for (int64_t core_index_x = index_x - core_radius; core_index_x <= index_x + core_radius; ++core_index_x) {
			...
			mass += (end - start);
			divider = (core_radius * 2 + 1) * (core_radius * 2 + 1);
			median_red /= divider;
	...
	std::cout << "Middle " << mass / (SizeX() * SizeY());
	delete[] pixels_tmp;
}
...
void Image::ParallelSmoothing(size_t core_radius) {
    auto** pixels_tmp = new Pixel*[size_x_];
    //Slowly
    for (int64_t index = 0; index < size_x_; ++index) {
        pixels_tmp[index] = new Pixel[size_y_];
    }
    //Equal
    for (int64_t index = 0; index < size_x_; ++index) {
	...
	int64_t median_red, median_green, median_blue, divider;
	double mass = 0;
	int64_t counter = 0;
    //Rapid
#pragma omp parallel for default(shared) private(median_red, median_green, median_blue, divider) reduction(+:mass)
	for (int64_t index_x = core_radius; index_x < size_x_ - core_radius; ++index_x) {
		for (int64_t index_y = core_radius; index_y < size_y_ - core_radius; ++index_y) {
            median_red = 0, median_green = 0, median_blue = 0;
			auto start = omp_get_wtime();
            for (int64_t core_index_y = index_y - core_radius; core_index_y <= index_y + core_radius; ++core_index_y) {
                for (int64_t core_index_x = index_x - core_radius; core_index_x <= index_x + core_radius; ++core_index_x) {
	...
	std::cout << "Middle " << mass / (SizeX() * SizeY()) / omp_get_num_threads();
    delete[] pixels_tmp;
}